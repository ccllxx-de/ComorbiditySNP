...
sys.stdout = open(os.path.join(output_dir, "model_resultsRR1diseaserowvectorsNN.txt"), "w")
...
cv_fold_df.to_csv(os.path.join(output_dir, "Metrics_PerModelRR1diseaserowvectorsNN.csv"), index=False)
...
for name, model in models.items():
    ...
    print(f"\n=== {name} Model Results RR1 diseaserowvectors ===")
...
# Save metrics to CSV
pd.DataFrame(all_metrics).to_csv(os.path.join(output_dir, "model_metricsRR1diseaserowvectorsNN.csv"), index=False)
