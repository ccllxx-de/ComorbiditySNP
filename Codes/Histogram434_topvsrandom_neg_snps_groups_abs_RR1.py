...
# 1. Load data
file_path = "/content/drive/MyDrive/topandrandom_neg_means_df.csv"
...
top_means = df[df['Group'] == 'Top negative SNPs']['Mean_negative_Effect'].abs()
random_means = df[df['Group'] == 'Random negative SNPs']['Mean_negative_Effect'].abs()
...
plt.hist(top_capped, bins=bins, histtype='step', color='#d62728',
         label=f'Top Negative SNPs (n={n_top})', linewidth=2.8, zorder=3)

# Random SNPs - color #1f77b4
plt.hist(random_capped, bins=bins, histtype='step', color='#1f77b4',
         label=f'Random Negative SNPs (n={n_random})', linewidth=2.8, zorder=3)
...
plt.title(f"Distribution of Mean Absolute Effect per SNP\n"
          f"{n_top} Top vs {n_random} Random Negative SNPs", fontsize=15, pad=20)
...
plt.tight_layout()
plt.savefig("/content/drive/MyDrive/neg_snps_absolute_effect_histogram.png")
plt.show()
