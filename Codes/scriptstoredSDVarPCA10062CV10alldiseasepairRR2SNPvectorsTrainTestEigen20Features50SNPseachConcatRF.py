...
sys.stdout = open(os.path.join(output_dir, "Concat_model_results_SNP_PCA_Importance_RF_RR2.txt"), "w")
...
cv_fold_df.to_csv(os.path.join(output_dir, "Concat_CV10Fold_Metrics_SNP_PCA_Detailed_RF_RR2.csv"), index=False)
...
pd.DataFrame(analysis_records).to_csv(os.path.join(output_dir, "RF_Top20_PC_to_SNP_Analysis_RF_RR2.csv"), index=False)

# Save the top 1000 SNPs list
pd.Series(top_1000_snps_list).to_csv(os.path.join(output_dir, "Top_1000_SNPs_Ranked_RF_RR2.csv"), index=False, header=['SNP_ID'])
...
metrics_df.to_csv(os.path.join(output_dir, "Concat_all_model_metrics_SNP_PCA_Final_RF_RR2.csv"), index=False)

# Print the final summary of the feature importance and SNP extraction
print("\n" + "="*80)
print("=== FEATURE IMPORTANCE AND SNP RANKING SUMMARY RF_RR2===")
print("="*80)
...
plt.title("Precision-Recall Curve SNP PCA 50 RF_RR2 (Concatenated Features)")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "pr_curve_SNP_PCA_Concat_RF_RR2.png"))
