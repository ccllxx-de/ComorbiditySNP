#W rows of disease vectors, RR4, Concat
import pandas as pd
import numpy as np
...
disease_pairs_df = pd.read_csv("./disease_pairs_with_labelsRR4.csv")
...
sys.stdout = open(os.path.join(output_dir, "Concat_model_resultsCV10alldiseasepairRR4diseaserowvectorsPCA100TrainTest.txt"), "w")

'''
pca_variance_df.to_csv(os.path.join(output_dir, "PCA_Variance_Reduction_SummaryRR4diseaserowvectorsCV10alldiseasepairSDVarPCA100TrainTest.csv"), index=False)
'''
...
cv_fold_df.to_csv(os.path.join(output_dir, "Concat_CV10Fold_Metrics_PerModelSDVarCV10RR4diseaserowvectorsalldiseasepairPCA100TrainTest.csv"), index=False)
...
for name, model in models.items():
    ...
    print(f"\n=== {name} Model Results RR4 diseaserowvectors CV10 PCA100 alldiseasepair ===")
...
# Save metrics to CSV
pd.DataFrame(all_metrics).to_csv(os.path.join(output_dir, "Concat_all_model_metricsCV10RR4diseaserowvectorsalldiseasepairPCA100TrainTest.csv"), index=False)
...
plt.title("Precision-Recall Curve PCA100 RR4 diseaserowvectors CV10 alldiseasepair")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "pr_curveRR4diseaserowvectorsCV10alldiseasepairTrainTestConcat.png"))
