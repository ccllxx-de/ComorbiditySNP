#W rows of disease vectors, RR2, Concat
import pandas as pd
import numpy as np
...
disease_pairs_df = pd.read_csv("./disease_pairs_with_labelsRR2.csv")
...
sys.stdout = open(os.path.join(output_dir, "Concat_model_resultsCV10alldiseasepairRR2diseaserowvectorsPCA100TrainTest.txt"), "w")

'''
pca_variance_df.to_csv(os.path.join(output_dir, "PCA_Variance_Reduction_SummaryRR2diseaserowvectorsCV10alldiseasepairSDVarPCA100TrainTest.csv"), index=False)
'''
...
cv_fold_df.to_csv(os.path.join(output_dir, "Concat_CV10Fold_Metrics_PerModelSDVarCV10RR2diseaserowvectorsalldiseasepairPCA100TrainTest.csv"), index=False)
...
for name, model in models.items():
    ...
    print(f"\n=== {name} Model Results RR2 diseaserowvectors CV10 PCA100 alldiseasepair ===")
...
# Save metrics to CSV
pd.DataFrame(all_metrics).to_csv(os.path.join(output_dir, "Concat_all_model_metricsCV10RR2diseaserowvectorsalldiseasepairPCA100TrainTest.csv"), index=False)
...
plt.title("Precision-Recall Curve PCA100 RR2 diseaserowvectors CV10 alldiseasepair")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "pr_curveRR2diseaserowvectorsCV10alldiseasepairTrainTestConcat.png"))
