...
# Randomly select 1500 disease pairs (same seed for reproducibility)
#disease_pairs_df = disease_pairs_df.sample(n=1500, random_state=42)

# no selection, all disease pairs
...
sys.stdout = open(os.path.join(output_dir, "Concat_model_resultsSDVarRR1CV10PCA100SNPvectorsalldiseasepairTrainTestNNtrapezoid.txt"), "w")		

'''
pca_variance_df.to_csv(os.path.join(output_dir, "PCA_Variance_Reduction_SummaryRR1SNPvectorsPCA100CValldiseasepairSDVarTrainTest.csv"), index=False)
'''
...
cv_fold_df.to_csv(os.path.join(output_dir, "Concat_CV10Fold_Metrics_PerModelSDVarCV10RR1SNPvectorsPCA100alldiseasepairTrainTestNNtrapezoid.csv"), index=False)
...
for name, model in models.items():
    ...
    print(f"\n=== {name} Model Results RR1 SNP vectors PCA100 CV10 alldiseasepair NNtrapezoid ===")
...
# Save metrics to CSV
pd.DataFrame(all_metrics).to_csv(os.path.join(output_dir, "Concat_all_model_metricsSDVarCV10RR1SNPvectorsPCA100alldiseasepairTrainTestNNtrapezoid.csv"), index=False)
...
plt.title("Precision-Recall Curve (PCA n_components=100) RR1 SNP vectors PCA100 CV10 alldiseasepair NNtrapezoid")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(os.path.join(output_dir, "pr_curveRR1SNPvectorsPCA100CValldiseasepairTrainTestConcatNNtrapezoid.png"))
